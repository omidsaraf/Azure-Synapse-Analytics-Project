...
## Identify the percentage of cash and credit card transactions by borough
````sql
WITH pay_type AS (
    SELECT 
...
SELECT
    ...
    CAST((SUM(CASE WHEN v_payment_type.payment_type_desc = 'Cash' THEN 1 ELSE 0 END) / CAST(COUNT(1) AS DECIMAL)) AS DECIMAL(5, 2)) AS cash_trip_pct,
    CAST((SUM(CASE WHEN v_payment_type.payment_type_desc = 'Credit card' THEN 1 ELSE 0 END) / CAST(COUNT(1) AS DECIMAL)) AS DECIMAL(5, 2)) AS card_trip_pct
FROM v_trip_data
...
![image](https://github.com/user-attachments/assets/70b65461-313d-489e-a114-520ce660d778)
